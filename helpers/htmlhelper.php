...
function watupro_cat_header($exam, $qct, $ques, $mode = 'show', $current_page = 1) {
	...
	if(!in_array($ques->cat_id, $question_catids)) {
		 if($qct and $mode == 'show') {
		 	$output .= "</div> 
		 	</div>"; // close the questions wrapper div and the previous category div
		 }	   	 	
   	 ...
   	 if(!empty($ques->cat_description)) $output .= "<div class='watupro-cat-description watupro-rcdiv'>".apply_filters('watupro_content', stripslashes($ques->cat_description))."</div>";
   	 
   	 $output .= "<div class='watupro-rcwrapper'>";
   	 $cat_count++;   	 
   }
