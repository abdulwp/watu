...
=======
	<div class="postbox watupro-tab-div" id="paymentsettings" style="display:none;">
		<div class="inside">
			<p><strong><?php printf(__("WatuPRO Intelligence module allows you to require payment to access selected %s.", 'watupro'), __('quizzes', 'watupro'))?></strong></p>
			<p><?php printf(__('If the existing payment methods are not sufficient you can get access to a lot more via the free <a href="%s" target="_blank">WooCommerce</a> plugin and our <a href="%s" target="_blank">free bridge</a>. If you choose this route you will be selling access to quizzes as products in your WooCommerce store.', 'watupro'), 'http://www.woothemes.com/woocommerce/', 'http://blog.calendarscripts.info/woocommerce-bridge-in-watupro/')?></p>
			<p><input type="checkbox" name="nodisplay_paid_quizzes" value="1" <?php if(get_option('watupro_nodisplay_paid_quizzes')) echo 'checked'?>> <?php printf(__('Do not display these %s in user dashboard until access to them is purchased (Users with admin rights will always see them).', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL)?></p>
			<p><input type="checkbox" name="paid_assign_groups" value="1" <?php if(get_option('watupro_paid_assign_groups') == 1) echo 'checked'?>> <?php printf(__('When access to a %1$s or a bundle is purchased automatically assign any required user groups or user roles. <a href="%2$s" target="_blank">(Learn more)</a>', 'watupro'), WATUPRO_QUIZ_WORD, 'https://blog.calendarscripts.info/watupro-automatically-assign-required-user-groups-or-roles-upon-payment')?></p>
         ...
         <?php endforeach; ?>
			...
			<?php if(!watupro_is_secure() and empty($use_pdt)):?>
					<p style="color:red;font-weight:bold;"><?php _e('Your site is not running on SSL so Paypal IPN will typically not work. You MUST use the PDT option below.', 'watupro');?></p>
			<?php endif;?>				
			...
			<div id="watuproStripe" style='display:<?php echo $accept_stripe ? 'block' : 'none';?>'>
				<p><b><?php printf(__('Note: direct Stripe payments are deprecated and new API versions will not be implemented at this time. We strongly recommend to use our free <a href="%s" target="_blank">WooCommerce integration</a> where you can flawlessly accept Stripe and many other payment methods.', 'watupro'), 'http://blog.calendarscripts.info/woocommerce-bridge-in-watupro/');?></b>
>>>>>>> branch/6.7.2
