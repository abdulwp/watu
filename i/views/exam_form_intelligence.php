    ...
    <div class="inside">
      	<h3><?php _e('Personality Quiz', 'watupro') ?></h3>
			...
			<input type="radio" name="user_choice" value="1" <?php if(!empty($advanced_settings['user_choice'])) echo 'checked'?> onclick="jQuery('#userChoiceCriteria').show();"> <?php printf(__('Let user choose what questions to answer (based on predefined criteria). This mode will not work with timed %s!', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL);?>
				<div id="userChoiceCriteria" style='display:<?php echo empty($advanced_settings['user_choice']) ? 'none' : 'block';?>'>
					<?php _e('Allow the following options:', 'watupro');?> &nbsp;
						<input type="checkbox" name="user_choice_modes[]" value="random_questions" <?php if(empty($advanced_settings['user_choice_enhanced']) or (!empty($user_choice_modes) and is_array($user_choice_modes) and in_array('random_questions', $user_choice_modes))) echo 'checked'?>> <?php printf(__('Select number of random questions from the whole %s', 'watupro'), WATUPRO_QUIZ_WORD);?>	
						&nbsp;
						<input type="checkbox" name="user_choice_modes[]" value="per_category" <?php if(empty($advanced_settings['user_choice_enhanced']) or (!empty($user_choice_modes) and is_array($user_choice_modes) and in_array('per_category', $user_choice_modes))) echo 'checked'?>> <?php printf(__('Select number of random questions per category', 'watupro'), WATUPRO_QUIZ_WORD);?>	
						&nbsp;
						<input type="checkbox" name="user_choice_modes[]" value="keywords" <?php if(empty($advanced_settings['user_choice_enhanced']) or (!empty($user_choice_modes) and is_array($user_choice_modes) and in_array('keywords', $user_choice_modes))) echo 'checked'?>> <?php printf(__('Select questions by keywords', 'watupro'), WATUPRO_QUIZ_WORD);?>		
						
					<p><?php printf(__('Alternatively you can override this whole setting by passing %1$s attribute to the %2$s shortcode with all the options like this: %3$s or just with some of the options like this: %4$s.', 'watupro'), 'user_choice', WATUPRO_QUIZ_WORD, '[watupro '.intval(@$dquiz->ID).' user_choice="random_questions, per_category, keywords"]', '[watupro '.intval(@$dquiz->ID).' user_choice="keywords, random_questions"]');?></p>						
				</div>			
			</p>
		   ...
		   <div id="dependencyDiv" style='display:<?php echo (empty($dquiz->ID) or !$dquiz->require_login)?'none':'block'; ?>'>
				...
				&nbsp;
				<span class="submit"><input type="button" value="<?php _e('Add This Dependency', 'watupro')?>" id="addDependencyButton" class="button button-primary"></span>
				</div>	
				...
				<?php foreach($dependencies as $dependency):?>
					...
					<option value="percent_points" <?php if(@$dependency->mode=='percent_points') echo 'selected'?>><?php _e('% of max. points', 'watupro')?></option>
                        ...
                        &nbsp;
						<span class="submit" id="delDepBtn<?php echo $dependency->ID?>"><input type="button" value="<?php _e('Mark To Delete', 'watupro')?>" onclick="WatuPRODep.del(true, <?php echo $dependency->ID?>);" class="button"></span>
						
						<span class="submit" id="restoreDepBtn<?php echo $dependency->ID?>" style="display:none;"><input type="button" value="<?php _e('Restore', 'watupro')?>" onclick="WatuPRODep.del(false, <?php echo $dependency->ID?>);" class="button"></span>
					</div>
				<?php endforeach;?>
		   ...
		   <div id="WatuPROPaymentDiv">
					<p><?php _e("Charge", 'watupro')?> <?php echo get_option("watupro_currency")?> <input type="text" name="fee" size="6" value="<?php echo empty($dquiz->fee) ? 0 : $dquiz->fee?>" onkeyup="wtpTogglePaidDiv(this.value);"> <?php _e("for accessing this test.", 'watupro')?> <b><?php printf(__('Note that users with rights to manage %s will always be able to access them for free.', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL)?></b><br>
					...
					<div style="margin-left:50px;display:<?php echo empty($dquiz->pay_always) ? 'none' : 'block'; ?>" id="wtpPayDecrease">
						...
						<?php printf(__('the payment amount with %1$s %2$s on each subsequent attempt.', 'watupro'), get_option("watupro_currency"), 
							'<input type="text" size="6" name="attempts_price_change_amt" value="'.(empty($advanced_settings['attempts_price_change_amt']) ? '' : $advanced_settings['attempts_price_change_amt']).'">');?> <br /> 
						<?php printf(__('The price will never go %1$s %2$s %3$s', 'watupro'), 
							'<span id="priceActionDir">'.((!empty($advanced_settings['attempts_price_change_action']) and $advanced_settings['attempts_price_change_action'] != 'increase') ? __('below', 'watupro') : __('above', 'watupro')).'</span>', 	
							get_option("watupro_currency"),
							'<input type="text" size="6" name="attempts_price_change_limit" value="'.(empty($advanced_settings['attempts_price_change_limit']) ? '' :$advanced_settings['attempts_price_change_limit']).'">');?>
					</div>
					...
					<div id="WatuPROWooDiv" style='display:<?php echo (!empty($dquiz->fee) and $dquiz->fee > 0) ? 'block' : 'none';?>'>
						<?php if(!empty($woo_products) and count($woo_products)):?>
						<p><?php _e('Sell as a WooCommerce product:', 'watupro');?> <select name="woo_product_id">
							<option value=""><?php _e('- Do not link to a product -', 'watupro');?></option>
							<?php foreach($woo_products as $product):?>
								<option value="<?php echo $product->ID?>" <?php if(!empty($advanced_settings['woo_product_id']) and $advanced_settings['woo_product_id'] == $product->ID) echo 'selected';?>><?php echo stripslashes($product->post_title);?></option>
							<?php endforeach;?>					
						</select></p>
					<?php endif;?>	
					
						<!-- we are just using the div but this is general payment-related feature and not strictly a WooCommerce related one -->
						<p><input type="checkbox" name="payment_restarts_attempts" value="1" <?php if(!empty($advanced_settings['payment_restarts_attempts'])) echo 'checked';?>> <?php _e('When a payment is made by a logged in user restart (for the user) any limits regarding number of allowed attempts or time elapsed for the next attempt.', 'watupro');?>
						<a href="https://blog.calendarscripts.info/watupro-intelligence-module-restart-quiz-attempts-when-payment-is-made/" target="_blank"><?php _e('(Learn more)', 'watupro');?></a></p>
					</div>
						
					
					<div id="WatuPROFreeAccessDiv" style='display:<?php (!empty($dquiz->fee) and $dquiz->fee > 0) ? 'block' : 'none';?>'>
						...
						<textarea name="payment_instructions" rows="3" cols="60"><?php echo empty($advanced_settings['payment_instructions']) ? '' : stripslashes(rawurldecode($advanced_settings['payment_instructions']));?></textarea></p>
						...
						<?php if($use_wp_roles == 1):?>
							<p><?php _e('Access remains free of charge for the following user roles (showing only roles that do not have management access):', 'watupro');?>
								<?php foreach($roles as $key => $role):
									...
									<span style="white-space:none;"><input type="checkbox" name="free_access_roles[]" value="<?php echo $key?>" <?php if(!empty($advanced_settings['free_access_groups']) and is_array($advanced_settings['free_access_groups']) and in_array($key, $advanced_settings['free_access_roles'])) echo 'checked'?>> <?php echo stripslashes($r->name)?></span>
								<?php endforeach;?>							
							</p>
						<?php else:?>
							...
							<?php if(!empty($groups) and count($groups)):
								...
								<span style="white-space:none;"><input type="checkbox" name="free_access_groups[]" value="<?php echo $group->ID?>" <?php if(!empty($advanced_settings['free_access_groups']) and is_array($advanced_settings['free_access_groups']) and in_array($group->ID, $advanced_settings['free_access_groups'])) echo 'checked'?>> <?php echo stripslashes($group->name)?></span>
							<?php endforeach;
							// allow other plugins to plug for groups here
							do_action('watupro-free-access', $groups, ( empty($dquiz->ID) ? null : $dquiz) ); 
		   ...
		   <?php if(count($editors) > 1 and $more_roles and current_user_can('manage_options')):?>
			   ...
			   <p><?php printf(__('This settings matters most if you select "Manage only %s created by users" on the <a href="%s" target="_blank">page that lets you fine-tune role settings</a>. Admin can always access all tests.', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL, 'admin.php?page=watupro_multiuser');?></p>
...
jQuery(function(){
	<?php if(count($other_exams)):?>
		...
		jQuery('#addDependencyButton').bind('click', function(){
			...
			// button
			var but = "<span class='submit'><input type='button' value='<?php _e("Delete", 'watupro')?>' onclick=\"jQuery('#dependencyRow"+rowNum+"').remove();\" class='button'></span>";
...
function wtpTogglePaidDiv(val) {
	if(val > 0) {
		jQuery('#WatuPROFreeAccessDiv').show();
		jQuery('#WatuPROWooDiv').show();
	} 
	else {
		jQuery('#WatuPROFreeAccessDiv').hide();
		jQuery('#WatuPROWooDiv').hide();
	}
} // end wtpTogglePaidDiv
</script>
