...
=======
	...
	<input type="text" name="reuse_title" value="<?php echo empty($advanced_settings['filter_reuse_title']) ? '' : $advanced_settings['filter_reuse_title'];?>"> 
	<?php _e('By internal comments:','watupro');?>
	<input type="text" name="reuse_comments" value="<?php echo empty($advanced_settings['filter_reuse_comments']) ? '' : $advanced_settings['filter_reuse_comments'];?>"> 
...
=======
	...
	<p><input type="checkbox" name="limit_reused_questions" value="1" <?php if(!empty($exam->limit_reused_questions)) echo 'checked';?>> <?php printf(__('Use only selected questions from the selected %s. (By default all questions are used)', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL);?></p>	
