	...
	<form method="post">
		...
		<input type="text" name="discount" size="4" value="<?php echo $coupon->discount?>">
			...
			<span id="dateCondition<?php echo $coupon->ID?>" style="display:<?php echo empty($coupon->date_condition) ? 'none' : 'inline'?>;">
				...
				<input type="text" name="end_date" class="watuproDatePicker" id="watuproDateEnd<?php echo $coupon->ID?>" value="<?php echo date_i18n($dateformat, strtotime(@$coupon->end_date))?>">
			</span>			
