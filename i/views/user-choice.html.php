	      ...
	      <p><input type="radio" name="watupro_mode" value="per_category" <?php if(empty($user_choice_radio_shown)) echo 'checked="true"';?> > <?php _e('Select questions from these question categories:', 'watupro');?>
	         <ul>
	            <?php foreach($all_cats as $cat):
                    $options = '';
                    for($i = 0; $i <= $cat->num_questions; $i++):
                        $options .= '<option value="'.$i.'">'.$i.'</option>';
                    endfor;?>
	              <li><?php if( $cat->parent_id ) echo ' --- ';
                    printf(__('Select %s questions from <b>%s</b> (%d total)', 'watupro'), 
	               '<select name="num_questions_'.$cat->ID.'">'.$options.'</select>', stripslashes($cat->name), $cat->num_questions);?></li>
	            <?php endforeach;?>
	         </ul>      
	      </p>   
