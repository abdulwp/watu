...
class WTPUser {
	...
	// automatically redirects the user to latest unfinished quiz on login
	public static function login_redirect($user_login, $user) {
		global $wpdb;
		
		if(get_option('watupro_redirect_current_taking') == 1) {
			// find the exam URL
			$current_taking = get_user_meta($user->ID, 'watupro_current_taking', true);
			
			if(!empty($current_taking)) {
				$exam_id = $wpdb->get_var($wpdb->prepare("SELECT exam_id FROM ".WATUPRO_TAKEN_EXAMS." WHERE in_progress=1 AND user_id=%d AND ID=%d",
					$user->ID, $current_taking));
					
				if(!empty($exam_id)) {
					// find the URL and redirect
					$url = WTPExam :: get_permalink($exam_id);
					
					if(!empty($url)) watupro_redirect($url);
				}	
			} // end if $current_taking
		} // end if option is ON
	} // end login_redirect
}
