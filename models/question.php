...
=======
	...
	static function prepare_vars(&$vars) {
      ...
      if(!empty($vars['compact_format']) and !empty($vars['compact_format_version']) and $vars['compact_format_version'] == 2 and $vars['answer_type'] != 'sort') $vars['compact_format'] = 3;
      if(!empty($vars['compact_format']) and !empty($vars['compact_format_version']) and $vars['compact_format_version'] == 3 and $vars['answer_type'] != 'sort') $vars['compact_format'] = 4;
...
=======
			if(preg_match("/^\|/", $vars['tags'])) $tags = str_replace(", ", "|", trim($vars['tags']) );
			else $tags = "|". str_replace(", ", "|", trim($vars['tags']) ) . "|";			
		}  
...
=======
      self :: prepare_vars($vars);
		...
		$elaborate_explanation = (empty($vars['do_elaborate_explanation']) or empty($vars['elaborate_explanation'])) ? '' : $vars['elaborate_explanation'];
		$tags = "|".str_replace(",", "|", str_replace(", ", "|", trim($vars['tags']) ) )."|";
>>>>>>> branch/6.7.2
...
<<<<<<< HEAD
=======
		// memory question? In this case let's make sure the part outside of the shortcode remains invisble at the beginning, so do the choices
		$hide_choices = false;
		if(strstr($ques->question, '[watupro-memory')) {
			$parts = explode('[/watupro-memory]', $ques->question);
			$ques->question = $parts[0].'[/watupro-memory]'.'<span class="watupro-memory-hidden" id="watuproMemoryContents'.$ques->ID.'">'.$parts[1].'</span>';
			$hide_choices = true;
		}
		$ques->hide_choices = $hide_choices;
...
=======
		// this line used watupro_nl2br instead of wpautop(). Now wpautop() seems no longer breaking tables with extra new lines?
 		...
 		echo "<input type='hidden' name='question_id[]' id='qID_{$question_count}' value='{$ques->ID}' class='watupro-question-id'/>";
 		echo "<input type='hidden' id='answerType{$ques->ID}' class='answerTypeCnt{$qct}' value='{$ques->answer_type}'>";
>>>>>>> branch/6.7.2
...
=======
  ...
  function display_choices($ques, $in_progress = null, $q_answers = null) {
     ...
     $rtl_class = empty(self :: $advanced_settings['is_rtl']) ? '' : 'watupro-rtl';
	  ...
	  // Memory hidden?
	  if(!empty($ques->hide_choices)) $rtl_class .= ' watupro-memory-hidden';	         
     ...
     if($ques->is_flashcard and $exam->is_likert_survey) $ques->is_flashcard = 0;
      ...
      switch($ans_type) {
      	case 'textarea':
      	 echo "<div class='question-choices $rtl_class' id='questionChoices".$ques->ID."'>";
>>>>>>> branch/6.7.2
      	 // open end question
...
=======
      		...
      		echo "<div class='question-choices $wrap_columns_class $rtl_class'  id='questionChoices".$ques->ID."'>";
...
=======
    	...
    	// when textareas have no possible answers, they are always correct when answered and incorrect when not answered
    	if($question->answer_type == 'textarea' and (!is_array($choices) or !count($choices))) {    			
    		$is_correct = $question->is_survey ? 0 : 1; // if survey - not correct
...
=======
				...
				$precision = 8;
				if(bccomp( number_format($points, $precision), number_format($max_points, $precision), 3) >= 0) $is_correct = 1;				
			}
...
=======
    	...
    	// num words?
    	if(strstr($feedback, '{{{num-words}}}') and !empty($ansArr[0])) {
    		$num_words = preg_split("/\s/", $ansArr[0]);
    		$feedback = str_replace("{{{num-words}}}", count($num_words), $feedback);
    	}
    	else $feedback = str_replace("{{{num-words}}}", 0, $feedback);
...
=======
			$num = count(explode('watupro-hint', $current_hints)) - 1;
>>>>>>> branch/6.7.2
...
=======
	 ...
	 static function flagged_for_review($question, $qct) {
		 	global $wpdb;
		 	$taking_id = (!empty($_POST['watupro_current_taking_id'])) ? intval($_POST['watupro_current_taking_id']) : 0;
