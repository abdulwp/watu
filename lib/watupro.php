...
=======
        ...
        if(empty($taking_id) and !empty($_COOKIE['watupro_taking_id_'.$exam_id]) and empty($user_ID)) $taking_id = intval($_COOKIE['watupro_taking_id_' . $exam_id]);	
     		...
     		// current page			
     		if($exam->single_page == WATUPRO_PAGINATE_ONE_PER_PAGE) $current_page = empty($_POST['current_question']) ? 1 : intval($_POST['current_question']);
...
=======
    ...
    // email exam details to where is selected
    // grade_id is passed to check if there is advanced setting that limits sending email to user
    function email_results($exam, $output, $grade_id = null, $certificate_id = null, $taking_id = null) {
    	   ...
    	   $sender = watupro_admin_email();
			...
			$output = apply_filters('watupro_email_vars', $output, $taking_id);
>>>>>>> branch/6.7.2
...
=======
			$attachments = watupro_maybe_attach_certificate( $certificate_id, $_POST['watupro_current_taking_id'] );
...
=======
    static function can_access($exam) { 
        ...
        if(!empty($advanced_settings['total_attempts_limit'])) {
        	  $num_attempts = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".WATUPRO_TAKEN_EXAMS." 
...
=======
		 ...
		 if(class_exists('WatuPROPlayLevels') and method_exists('WatuPROPlayLevels', 'can_access')) {
		 	 if(!WatuPROPlayLevels :: can_access($exam)) return false;
		 	 if(method_exists('WatuPROPlayBadges', 'can_access') and !WatuPROPlayBadges :: can_access($exam)) return false;
>>>>>>> branch/6.7.2
		 }
...
=======
			$output = str_replace([ '<span class="user-answer">', '<span class="user-answer correct-gap-answer">', '<span class="user-answer wrong-gap-answer">' ], "<span ".$user_answer_style.">", $output);
>>>>>>> branch/6.7.2
...
=======
		...
		$advanced_settings = unserialize(stripslashes($exam->advanced_settings));
		// admins can always see the table
		if(current_user_can(WATUPRO_MANAGE_CAPS)) $advanced_settings['show_only_snapshot'] = null;
		// but if we are not admin and not user, this means we see it because of $advanced_settings['allow_public_details']. Then $advanced_settings['show_only_snapshot']  is always true
		if(!current_user_can(WATUPRO_MANAGE_CAPS) and $student->ID != $user_ID and !empty($advanced_settings['allow_public_details'])) {
			$advanced_settings['show_only_snapshot'] = 1;
		} 
...
=======
   ...
   $_watu = new WatuPRO();
   $taking_id = $_watu->add_taking($_POST['exam_id'],1);
   $answer = isset($_POST['answer-'.$_POST['question_id']]) ? $_POST['answer-'.$_POST['question_id']] : '';
	$answer = serialize($answer);
   
   if(!empty($user_ID)) update_user_meta($user_ID, 'watupro_current_taking', $taking_id);
>>>>>>> branch/6.7.2
...
=======
	...
	update_user_meta($user_ID, 'watupro_current_taking', $taking_id);
