<<<<<<< HEAD
<?php if(empty($in_shortcode)):?><div class="wrap">
=======
...
<<<<<<< HEAD
	<?php if(sizeof($certificates)):?>
		...
		<table class="widefat">
			...
			<th><?php _e('View/Print', 'watupro')?></th><?php if(empty($in_shortcode)):?><th><?php _e('Allow public access', 'watupro')?></th><?php endif;?></tr>
=======
	<?php if(count($certificates)):?>
		
		<table class="widefat">
			<tr><th><?php printf(__('%s title', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD))?></th><th><?php _e('Completed on', 'watupro')?></th><th><?php _e('With result', 'watupro')?></th>
...
<<<<<<< HEAD
				...
				<?php if(empty($in_shortcode)):?><td align="center"><input type="checkbox" onclick="window.location = 'admin.php?page=watupro_my_certificates&set_public_access=1&id=<?php echo $certificate->us_id?>&public_access=' + (this.checked ? 1 : 0);" <?php if(!empty($certificate->public_access)) echo 'checked'?>></td><?php endif;?></tr>
=======
				<td><?php echo "<a href='".site_url("?watupro_view_certificate=1&taking_id=".WatuPROCertificate :: obfuscate($certificate->taking_id)."&id=".$certificate->ID)."' target='_blank'>".__('print your certificate', 'watupro')."</a>"?></td>
