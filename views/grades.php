<div class="wrap watupro-wrap">
	...
	<p><?php _e('Currently managing grades for', 'watupro')?> <select name="grades_by_percent" onchange="window.location='admin.php?page=watupro_default_grades&percentage_based=' + this.value;">
			 ...
			 <?php printf(__('This is not a setting - it is just a selector that lets you switch between managing grades for percent based or point based quizzes. The selection whether a quiz is point based or percent based is done on the Edit %1$s page itself.', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD));?><br>
   ...
   <?php endif;?>
	...
	<?php foreach($grades as $grade):
		if($grade->is_cumulative_grade and empty($exam->is_personality_quiz)) continue;
		if($grade->is_cumulative_grade and watupro_intel()):?>
			<p class="help watupro-help"><?php printf(__('Note: this is an automatically created cumulative personality type. These are created when a %1%s is completed with multiple personality types ranking on top. For performance reasons they are generated only once so if you have made any changes to the individual personalities after this one was generated you may want to apply the changes to it too.', 'watupro'), WATUPRO_QUIZ_WORD);?></p>
		<?php endif;?>
		<form method="post" class="watupro-form" onsubmit="return validateGrade(this);">
			<div class="watupro-padded">
