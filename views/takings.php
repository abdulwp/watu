...
<div class="wrap watupro-wrap">
	...
	<?php if(!empty($exam->ID)):?>		
		...
		<?php endif;?>
		| <a href="admin.php?page=watupro_import_takings&exam_id=<?php echo $exam->ID?>"><?php printf(__('Import %s results', 'watupro'), WATUPRO_QUIZ_WORD)?></a>
	...
	<?php endif; // if(!empty($exam->ID))?>
			<p><?php _e('Shortcode to publish a simplified version of this page:', 'watupro');?> <input type="text" size="30" value='[watupro-takings quiz_id=<?php echo intval(@$exam->ID)?> wrap=1]' readonly="readonly" onclick="this.select();">
	...
	<input type="hidden" name="page" value="watupro_takings">
		...
		<div><label><?php _e('% from max. points', 'watupro')?></label> <select name="percent_pointsf">
			<option value="equals" <?php if(empty($_GET['percent_pointsf']) or $_GET['percent_pointsf']=='equals') echo "selected"?>><?php _e('Equal', 'watupro')?></option>
			<option value="less" <?php if(!empty($_GET['percent_pointsf']) and $_GET['percent_pointsf']=='less') echo "selected"?>><?php _e('Is less than', 'watupro')?></option>
			<option value="more" <?php if(!empty($_GET['percent_pointsf']) and $_GET['percent_pointsf']=='more') echo "selected"?>><?php _e('Is more than', 'watupro')?></option>			
		</select> <input type="text" name="percent_points" value="<?php echo empty($_GET['percent_points']) ? '' : stripslashes(esc_attr($_GET['percent_points']))?>"></div>		
		...
		<?php if(!empty($namaste_courses)):?>
			<div><label><?php _e('Student in course:', 'watupro')?></label> <select name="namaste_course_id" onchange="this.value ? jQuery('#namasteCourseTags').show() : jQuery('#namasteCourseTags').hide();">
			...
			<span id="namasteCourseTags" style='display: <?php echo empty($_GET['namaste_course_id']) ? 'none' : 'inline-block'?>'>
				<?php _e('Tagged as:', 'watupro');?> <input type="text" name="namaste_tags" value="<?php echo $_GET['namaste_tags'] ?? ''?>" placeholder="tag1, tag2">
			</span>			
			</div>
		<?php endif;?>
		...
		<div><p><input type="checkbox" name="filter_by_question" value="1" <?php if(!empty($_GET['filter_by_question'])) echo 'checked'?> onclick="WatuPROFilterByQuestion(this);"> <?php _e('Filter by answer to a specific question (currently only single-choice questions)', 'watupro');?></p></div>
	...
	<?php else:?>
		...
		<form method="post">
		<?php if($in_shortcode and !empty($atts['wrap'])):?><div style="overflow-x: auto;"><?php endif;?>
		...
		<?php foreach($takings as $taking):
			...
			<?php // We are in shortcode but not admin. $atts['public'] and $atts['show_details'] are allowed, and the quiz advanced settings also allows to view
			if($in_shortcode 
				and (!is_user_logged_in() or (!current_user_can('watupro_manage_exams') and !current_user_can('manage_options')))
				and !empty($atts['public']) and !empty($atts['show_details']) and !empty($advanced_settings['allow_public_details'])):
					if(!empty($atts['details_no_popup'])):
						$taking_url = add_query_arg('watupro_taking_id', $taking->ID, $_GET['current_url']);?>
						<td><a href="<?php echo $taking_url?>"><?php _e('view', 'watupro')?></a></td><?php
					else: // details popup?>
					<td><a href="#" onclick="WatuPRO.takingDetails('<?php echo $taking->ID?>');return false;"><?php _e('view', 'watupro')?></a></td><?php
					endif; // end details_no_popup case			
			endif; // end the specific case when public can view details 		
			
			// regular view/edit case (not in shortcode or in shortcode but for managers)
			if(!$in_shortcode or (is_user_logged_in() and (current_user_can('watupro_manage_exams') or current_user_can('manage_options')))):?>		
		...
		<?php if($in_shortcode and !empty($atts['wrap'])):?></div><?php endif;?>
