...
<<<<<<< HEAD
	...
	&nbsp;
	<?php _e('title contains:', 'watupro')?> <input type="text" name="title" value="<?php echo empty($_GET['title']) ? '' : $_GET['title']?>">
...
<table class="widefat">
	...
	<tr>
		...
		<?php if(empty($low_memory_mode) or $low_memory_mode != 1):?><th scope="col"><?php _e('No. questions', 'watupro') ?></th><?php endif;?>
		...
		<?php if($multiuser_access != 'view' and $multiuser_access != 'group_view'):?>
...
=======
	...
	&nbsp;
	<?php _e('Status:', 'watupro')?> <select name="is_active">
		<option value="-1"><?php _e('All', 'watupro')?></option>
		<option value="1" <?php if(isset($_GET['is_active']) and $_GET['is_active'] === '1') echo 'selected'?>><?php _e('Active', 'watupro')?></option>
		<option value="0" <?php if(isset($_GET['is_active']) and $_GET['is_active'] === '0') echo 'selected'?>><?php _e('Inactive', 'watupro')?></option>
	</select>
	&nbsp;
	...
	<input type="submit" value="<?php printf(__('Filter %s', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL);?>" class="button-primary">
	
	<?php if(!empty($filter_sql)):?><input type="button" value="<?php _e('Clear filters', 'watupro')?>" onclick="window.location='admin.php?page=watupro_exams'" class="button"><?php endif;?></p>
</form>
...
<<<<<<< HEAD
		...
		if(!empty($advanced_settings['admin_comments'])):
			echo "<p>".nl2br(stripslashes(base64_decode($advanced_settings['admin_comments'])))."</p>";
		endif;?></td>
        <td><input type="text" size="14" value="[watupro <?php echo $quiz->ID ?>]" onclick="this.select();" readonly></td>
		<?php if(empty($low_memory_mode) or $low_memory_mode != 1):?><td><?php echo empty($quiz->reuse_questions_from) ? $quiz->question_count : __('Reuses from other test(s)', 'watupro')?></td><?php endif;?>
=======
...
<<<<<<< HEAD
		<?php if($multiuser_access != 'view' and $multiuser_access != 'group_view'):?>
			...
			<td><a href="<?php echo wp_nonce_url('admin.php?page=watupro_exams&amp;action=delete&amp;quiz='.$quiz->ID, 'delete_quiz', 'delete_nonce')?>" class='delete' onclick="return confirm('<?php echo  addslashes(__("You are about to delete this quiz? This will delete all the questions and answers within this quiz. Press 'OK' to delete and 'Cancel' to stop.", 'watupro'))?>');"><?php _e('Delete', 'watupro')?></a></td>
		<?php endif;?>	
		</tr>
<?php endforeach;?>
...
=======
	...
	<div align="center" style="display:none;" id="massQuizActions">
		<p>
		<?php if(count($cats)): 
		...
		<p>
			<?php _e('Other mass changes:', 'watupro');?>
			<?php _e('Direction:', 'watupro');?> <select name="is_rtl">
				<option value="0"><?php _e('Left to right', 'watupro');?></option>
				<option value="1"><?php _e('Right to left', 'watupro');?></option>
			</select>
			
			<input type="submit" name="other_mass_changes" onclick="if(!confirm('<?php _e('Are you sure?', 'watupro')?>')) return false;" value="<?php _e('Save Mass Changes', 'watupro')?>" class="button-primary">
		</p>
	</div>
