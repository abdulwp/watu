<div class="wrap watupro-wrap">
	...
	<form method="post">
		<h2><?php _e('Facebook and LinkedIn Sharing', 'watupro')?></h2>
		...
		<div id="linkedinOptions">
			...
			<br>
			<b><?php _e('Known issue: Facebook no longer supports sharing a pre-defined text.', 'watupro');?></b></p>		
		...
		<div id="twitterOptions" style="display:<?php echo empty($twitter_options['use_twitter']) ? 'none' : 'block'?>">
			...
			<p><?php _e('Via @', 'watupro')?> <input type="text" name="via" value="<?php echo esc_attr($twitter_options['via'] ?? '')?>"></p>
			<p><?php _e('Hashtag #', 'watupro')?><input type="text" name="hashtag" value="<?php echo esc_attr($twitter_options['hashtag'] ?? '')?>"></p>
			<p><?php _e('Tweet text (No more than 140 chars):', 'watupro')?> <textarea name="tweet" maxlength="140" rows="3" cols="40"><?php echo esc_textarea($twitter_options['tweet'] ?? '')?></textarea>
