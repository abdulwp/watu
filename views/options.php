			...
			<div class="inside" style="padding:8px">
				...
				<div class="inside" style="padding:8px">
					<p><input type="checkbox" name="auto_db_cleanup" value="1" <?php if(get_option('watupro_auto_db_cleanup')=='1') echo 'checked'?>> 					
						...
						<?php echo do_action('watupro-external-cleanup-config', $db_cleanup_config);?>
						</p>
						...
						<?php echo do_action('watupro-external-blankout-config', $db_blankout_config);?>
						</p>
	...
	<div class="postbox watupro-tab-div" id="usersettings" style="display:none;">
		<div class="inside">
			...
			<?php endif;?>
				...
				<p><input type="checkbox" name="redirect_current_taking" value="1" <?php if(get_option('watupro_redirect_current_taking') == 1) echo 'checked'?>> <?php printf(__('When user logs in, automatically redirect them to their latest unfinished %s attempt (if any)', 'watupro'), WATUPRO_QUIZ_WORD);?></p>
				
			<h4><?php printf(__('My %s Page', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD_PLURAL));?></h4>
			...
			<div id="detailsDefaultView" style='display:<?php echo (get_option('watupro_nodisplay_myquizzes') == 1) ? 'none' : 'block';?>'><?php _e('Default view for "View details" pop-up:', 'watupro');?> <select name="taking_details_default_view">
				...
				<?php _e('Default download view:', 'watupro');?> <select name="taking_details_default_download">
				  ...
				  <p><?php _e('Format of the "%s to complete" section:', 'watupro');?> <select name="myquizzes_grouped_by_cat">
				  	  <option value="0"><?php printf(__('Ungrouped, sorted by %s name (default)', 'watupro'), WATUPRO_QUIZ_WORD);?></option>
				  	  <option value="1" <?php if($myquizzes_grouped_by_cat == 1 and $myquizzes_grouped_by_cat == 1) echo 'selected';?>><?php printf(__('Grouped by category', 'watupro'), WATUPRO_QUIZ_WORD);?></option>
				  </select></p>
			</div>
			...
			<h4><?php _e('Other user dashboard pages', 'watupro');?></h4>			
			...
			<h4><?php _e('Other user settings', 'watupro');?></h4>
