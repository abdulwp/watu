...
<div class="wrap watupro-wrap">
	...
	<p><?php _e('Because most of the Watu PRO screens are self-explaining, this page is not meant to be a comprehensive user manual. Its intent is only to further clarify some of the functionality in the plugin.', 'watupro');?> <br>	
	
	<?php _e('See the <b><a href="https://blog.calendarscripts.info/watupro-user-manual/" target="_blank">full User Manual</a></b> here or check the quick <a href="http://calendarscripts.info/watupro/howto.html" target="_blank">online FAQ page</a>', 'watupro')?></p>
...
=======
	...
	<p><strong><?php _e('Here is how to start really quick:', 'watupro');?></strong> (<a href="https://www.youtube.com/watch?v=eiVNOWJOVUs" target="_blank"><?php _e('Video', 'watupro');?></a> / <a href="http://blog.calendarscripts.info/watupro-quick-getting-started-guide/" target="_blank"><?php _e('Pictorial', 'watupro');?></a>)</p>
	<ol>
...
=======
		...
		<li><input type="text" value='[watuprolist cat_id="ALL"]' onclick="this.select();" readonly="readonly"> <?php _e('lists links to all published quizzes in the system.', 'watupro')?> <?php _e('Use [watuprolist cat_id="ALL" orderby="title"] to show them ordered by title or [watuprolist cat_id="ALL" orderby="latest"] to order them latest on top. The same format can be used in the above shortcode as well.', 'watupro')?> 
		<br>
		<?php printf(__('You can display a drop-down with all categories and let user choose. Pass the attribute <b>%s</b>.', 'watupro'), 'show_dropdown=1');?>		<br>
		<span style="color:red;"><b><?php printf(__('The shortcode will NOT display %s that are not published!', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL);?></b></span> <?php printf(__('A published %s is a %s whose shortcode is placed inside a post or page - either manually or using the "auto publish" option.', 'watupro'), WATUPRO_QUIZ_WORD, WATUPRO_QUIZ_WORD);?> <br><a href="https://blog.calendarscripts.info/list-quizzes-in-watupro" target="_blank"><?php _e('Learn more and see examples', 'watupro');?></a></li>
		...
		<li><input type="text" value="[watupro-mysettings]" onclick="this.select();" readonly="readonly"> <?php printf(__('show the logged in user "%s Settings" page.', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD));?> </li>
		...
		<?php printf(__('The shortcode <b>%1$s</b> will return the position of the currently logged in user in the leaderboard. You can pass the parameter "user_id" if you want to specify other user ID', 'watupro'), '<input type="text" size="25" value="[watupro-leaderboard-position]" onclick="this.select()" readonly="readonly">');?></li>
		...
		<li><input type="text" value='[watupro-multi-quiz][/watupro-multi-quiz]' onclick="this.select();" readonly="readonly"> <?php printf(__('This shortcode allows you to show cumulative stats from logged in user attempts on multiple %1$s. It also lets you display conditional content based on the achieved results. Learn everything about this shortcode <a href="%2$s" target="_blank">here</a>.', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL, 'https://blog.calendarscripts.info/performance-on-multiple-quizzes-and-conditional-content-in-watupro/');?></li>
	</ul>
	...
	<ul>
		<li><input type="text" value="[watupror overview]" onclick="this.select();" readonly="readonly"> <?php _e('or', 'watupro');?> <input type="text" value="[watupror overview X]" onclick="this.select();" readonly="readonly"> <?php _e('shows the Overview page from the reports. You can pass user ID for X or leave it empty to display reports to the currently logged user.', 'watupro');?>
		<br><?php printf(__('If the charts do not look as expected, try the GD mode by passing parameter %1$s to the shortcode. Example: %2$s.','watupro'), 'charts_gd=1','[watupror overview charts_gd=1]');?></li>
...
chart on all question types except on "single answer" and "multiple answer" questions where it loads chart per each answer. You can force it to always show correct / incorrect by adding parameter to the shortcode:', 'watupro')?> [watupro-poll question_id="X" mode="correct"]. <?php _e('You can also control the colors used in the chart like this:', 'watupro');?> [watupror-poll question_id="X" correct_color="green" wrong_color="#FF0000"]. <br>
	  <?php printf(__('The optional parameter <b>%1$s</b> lets you show which is the current user answer when the shortcode is used in the "Final page". You can pass any text or even HTML code (when using HTML make sure the rich text editor is in Text mode) and it will be shown next to the corresponding answer(s) or correct / incorrect stats. If you pass "CHECK" to the attribute we will generate a checkmark. Example: <b>%2$s</b>', 'watupro'), 'user_choice', '[watupror-poll question_id="X" user_choice="CHECK"]');?><br>
	  <?php printf(__('You can make the poll show answers for a selected user group by passing the parameter <b>%s</b>. Replace X with the actual group ID.','watupro'), "group_id=X");?><br>
	  <?php printf(__('By default the shrotcode produces a horizontal bar chart. You can use the parameter "orientation" to make it a vertical one. Example: <b>%s</b>', 'watupro'), '[watupror-poll question_id="X" orientation="vertical"]');?></li>
		...
		<li><input type="text" value='[watupror-quiz-cat-chart quiz_id=X]' onclick="this.select();" readonly="readonly" size="25"> <?php printf(__("Generates a bar chart from everyone's performance on the %s. The shortcode accepts exactly the same parameters as the above shortcode but requires the parameter quiz_id. Note that drawing the chart for a %s that has been completed by tens of thousands of users may require a lot of server memory.", 'watupro'), WATUPRO_QUIZ_WORD, WATUPRO_QUIZ_WORD);?><br>
		<?php printf(__('The parameter <b>%s</b> will sum up the subcategory performance into the parent categories and will not generate pies for the subcategories.', 'watupro'), 'sum_subcategories=1');?><br> 
		<?php printf(__('The parameter <b>%s</b> will create two bars for each category: one for the results of the current attempt and one for average results from everyone.', 'watupro'), 'compare=1');?><br> 		
		</li>
		<li><input type="text" value='[watupror-pie-chart]' onclick="this.select();" readonly="readonly"> <?php printf(__('generates a pie chart from the user performance per question category in a single quiz attempt. The shortcode is typically used in the Quiz Output to show chart from the just submitted %s. You can however pass "taking_id" parameter if you want to use it elsewhere. Other parameters accepted: "from" - "points" (default, will generate a single pie chart based on points collected per category), "correct" (will show one pie per each category, showing %% of correct vs. incorrect answers), or "questions" (will generate a single pie chart from correct / wrong / unanswered questions on the whole %s). You can also pass parameter "radius" to define the radius of the pie chart in pixes.', 'watupro'), WATUPRO_QUIZ_WORD, WATUPRO_QUIZ_WORD);?> <br>
		<?php printf(__('Passing the parameter <b>%s</b> when "from" is "correct" will generate charts with 3 sectors - for correct, answered, and unanswered questions.', 'watupro'), 'draw_unanswered=1');?><br>
		<?php printf(__('The parameter <b>%s</b> will sum up the subcategory performance into the parent categories and will not generate pies for the subcategories.', 'watupro'), 'sum_subcategories=1');?><br>
		...
		<p><?php printf(__('<b>This chart can also be based on all logged in user attempts from all %1$s.</b> In this case the "from" parameter cannot be "questions" because different %2$s can contain different questions. To get the chart show user performance per question categories globally just pass the parameter %3$s.', 'watupro'), WATUPRO_QUIZ_WORD_PLURAL, WATUPRO_QUIZ_WORD_PLURAL, 'taking_id="ALL"');?></p>
		<p><?php printf(__('<b>This chart can also be based on the latest attempt for the logged in user on a specified %1$s.</b> In this case it can be used outside of the "final screen". The parameter <b>%2$s</b> should not be present at all. You have to include the parameter <b>%3$s</b>.', 'watupro'), WATUPRO_QUIZ_WORD, 'taking_id', 'quiz_id');?></p>
		<p><?php printf(__('<a href="%s"target="_blank">See here</a> a couple of examples of these charts.', 'watupro'), 'https://blog.calendarscripts.info/using-category-grades-in-watu-pro/#charts');?></p>
		</li>
		...
		<li><input type="text" value='[watupror-performance-chart]' onclick="this.select();" readonly="readonly"> <?php printf(__('This is a simple chart showing your points collected vs. the maximum points that you could achieve in a single quiz attempt. See all parameters <a href="%s" target="_blank">here</a>.', 'watupro'), 'https://blog.calendarscripts.info/charts-in-watupro-reporting-module/#basicperformance');?></li>
		<li><input type="text" value='[watupror-questions-performance-chart]' onclick="this.select();" readonly="readonly"> <?php printf(__('TSimilar to the above chart, this one shows your points as a percentage of the maximum points but for every question answered in the %1$s. See all parameters <a href="%2$s" target="_blank">here</a>.', 'watupro'), WATUPRO_QUIZ_WORD, 'https://blog.calendarscripts.info/charts-in-watupro-reporting-module/#questionperformance');?></li>
	</ul>
	<?php endif;?>
	...
	<p><?php _e("If you have any questions or issues please send us email at the emails shown below this paragraph. <strong>VERY IMPORTANT:</strong> if you have a problem while using the plugin please <b>do provide the URL where we can observe it</b>. Seeing the URL helps far more than sending long descriptions of the problem. We don't knowingly release anything buggy so you can assume we don't know about the problem you have until you send us your report. Please keep your support emails short.", 'watupro');?></p>
	
	<p><input type="checkbox" id="understandURL"> <?php _e('I understand that if I am requesting support about an issue on my site, my message must contain the URL where the problem can be seen.', 'watupro');?></p>
	<p><input type="button" value="<?php _e('See support email addresses', 'watupro');?>" onclick="revealSupportEmails()" class="button button-primary"></p>
	
	<p id="supportEmails" style="display:none;"><a href="mailto:info@calendarscripts.info">info@calendarscripts.info</a> or <a href="mailto:support@kibokolabs.com">support@kibokolabs.com</a></p>
</div>	

<script>
function revealSupportEmails() {
    let understandURL = document.getElementById('understandURL');
    if( !understandURL.checked ) {
        alert("<?php _e('Please read and confirm the notice about sending an URL.', 'watupro');?>");
        understandURL.focus();
        return false;
    }
    
    document.getElementById('supportEmails').style.display="block";
}
</script>
>>>>>>> branch/6.7.2
