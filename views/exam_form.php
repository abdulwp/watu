...
<div id="poststuff">
		 ...
		 <div id="alwaysShowSubmit" <?php if(!empty($dquiz->ID) and $dquiz->single_page==1) echo "style='display:none;'"?>>			
			...
			<p><input type="checkbox" name="show_progress_bar" value="1" <?php if(!empty($advanced_settings['show_progress_bar'])) echo "checked"?> onclick="this.checked ? jQuery('#progressBarPercent').show() : jQuery('#progressBarPercent').hide();"> <?php _e('Show progress bar on top', 'watupro')?>
				<span id="progressBarPercent" style='display:<?php echo empty($advanced_settings['show_progress_bar']) ? 'none' : 'inline';?>;'>
					...
					<input type="checkbox" name="progress_bar_mode" value="1" <?php if(!empty($advanced_settings['progress_bar_mode'])) echo "checked"?>> <?php printf(__('Progress calculation is based on %% of answered questions from all instead of current location in the %s (the default behavior).', 'watupro'), WATUPRO_QUIZ_WORD);?>			
				</span>			
			</p>	 
		...
		<select name="design_theme">		
          ...
          <br>               
			...
			<div style='display:<?php echo empty($dquiz->is_scheduled) ? 'none': 'block';?>' id="scheduleSettings"><label><?php _e('Schedule from:', 'watupro')?></label> &emsp;
	                <input type="date" name="schedule_from" value="<?php echo $schedule_from?>">
	                &nbsp;
	                ...
	                <input type="date" name="schedule_to" value="<?php echo $schedule_to?>">
	                &nbsp;
			...
			<br />
			<?php do_action('watupro_edit_quiz_general_tab', $dquiz, $advanced_settings);?>
		</div>
	</div>
	...
	<div class="postbox watupro-tab-div" id="useremailsettings" style="display:none;">	 
	 <div class="inside">
	    ...
	    <div id="loginMode" style='margin-left:20px;display:<?php echo !empty($dquiz->require_login)?'block':'none';?>'> 
	        ...
	        do_action('watupro_quiz_form_login_required_badges', (empty($advanced_settings['play_badges']) ? array() : $advanced_settings['play_badges']));?>	
		...
		<p><input type="checkbox" name="email_taker" value="1" <?php if(!empty($dquiz->email_taker)) echo "checked"?> onclick="this.checked ? jQuery('#emailTakerOptions, #emailWarning').show() : jQuery('#emailTakerOptions, #emailWarning').hide();"> <?php _e('Send email to the user with their results', 'watupro')?></p>
	    	...
	    	<div class="inside">
		    	...
		    	<div id="askForContactDetails" style='display:<?php echo empty($advanced_settings['ask_for_contact_details']) ? 'none' : 'block';?>'>
					<p id="watuProAskTimerNote" style='display:<?php echo (!empty($dquiz->time_limit) and $dquiz->time_limit > 0 and !empty($advanced_settings['ask_for_contact_details']) and $advanced_settings['ask_for_contact_details']=='end') ? 'block' : 'none';?>'>
			    	...
			    	<p><label><?php _e('Value (text) of the "Start" button:', 'watupro')?></label> <input type="text" name="ask_for_start_button" value="<?php echo empty($advanced_settings['contact_fields']['start_button']) ? sprintf(__('Start %s!', 'watupro'), ucfirst(WATUPRO_QUIZ_WORD)) : htmlentities(stripslashes(rawurldecode($advanced_settings['contact_fields']['start_button'])))?>">
	...
	<div id="finalscreen" class="watupro-tab-div postbox" style="display:none;">				
		<div class="inside">
            ...
            <div id="finalScreenUnique" style='display:<?php echo empty($advanced_settings['use_default_final_screen'])?'block':'none';?>'>
                <?php wp_editor($final_screen, "watupro_content", array("editor_class" => 'i18n-multilingual', 'textarea_name' => 'content')); ?>
			</div>
			
			 <p><input type="checkbox" name="use_default_final_screen" value="1" <?php if(!empty($advanced_settings['use_default_final_screen'])) echo "checked"?> onclick="this.checked?jQuery('#finalScreenUnique').hide():jQuery('#finalScreenUnique').show()"> <?php _e('Use the default final screen from the Watu PRO settings page', 'watupro')?></p>
			...
			<p><input type="checkbox" name="use_different_email_output" value="1" <?php if(!empty($dquiz->email_output) or !empty($advanced_settings['use_default_email_output'])) echo "checked"?> onclick="this.checked?jQuery('#emailOutput').show():jQuery('#emailOutput').hide()"> <?php _e('Use different output for the email that is sent to the user/admin', 'watupro')?></p>
			
			<div id="emailOutput" style='display:<?php echo ( empty($dquiz->email_output) and empty($advanced_settings['use_default_email_output']) ) ? 'none':'block';?>'>
                ...
                <p><label><?php _e('Email contents:', 'watupro');?></label>
                
                <div id="emailOutputUnique" style='display:<?php echo empty($advanced_settings['use_default_email_output'])?'block':'none';?>'>
                    <?php wp_editor(stripslashes($dquiz->email_output),"email_output", array("editor_class" => 'i18n-multilingual')); ?></p>
                </div>
                ...
                <p><input type="checkbox" name="use_default_email_output" value="1" <?php if(!empty($advanced_settings['use_default_email_output'])) echo "checked"?> onclick="this.checked?jQuery('#emailOutputUnique').hide():jQuery('#emailOutputUnique').show()"> <?php _e('Use the default email output from the Watu PRO settings page', 'watupro')?></p>
			</div>
