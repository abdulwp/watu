...
<<<<<<< HEAD
	<table class="widefat">
		...
		<?php foreach($users as $user):
			...
			if($certificate->is_multi_quiz):  printf(__('Avg. points: %d; Avg. %% correct answers: %d%%', 'watupro'), $user->avg_points, $user->avg_percent);
		...
		<tbody>
			<?php foreach($users as $user):
				...
				<?php if($certificate->require_approval):?>
					...
					<?php else: _e('Approved', 'watupro'); endif;?></td>
				<?php endif;?>
				<td><a href="<?php echo site_url('?watupro_view_certificate=1&taking_id='.WatuPROCertificate :: obfuscate($user->taking_id).'&id='.$certificate->ID)?>" target="_blank"><?php _e('View / print', 'watupro')?></a></td>
				<td><a href="#" onclick="watuPRORemoveUserCertificate(<?php echo $user->user_certificate_id?>);return false;"><?php _e('Remove', 'watupro')?></a></td></tr>
			<?php endforeach;?>
		</tbody>
>>>>>>> branch/6.7.2
