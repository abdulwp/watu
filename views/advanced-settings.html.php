	...
	<div class="inside">
			...
			<div>
				<p><?php printf(__('Allow total of %1$s attempts on this %2$s (enter 0 for no limit).', 'watupro'), '<input type="text" name="total_attempts_limit" value="'.(empty($advanced_settings['total_attempts_limit']) ? 0 : $advanced_settings['total_attempts_limit']).'" size="4" id="totalAttemptsLimit">', WATUPRO_QUIZ_WORD);?> <b><?php _e('This limit is for all attempts made by everyone. It is not per user.', 'watupro');?></b></p>
			...
			<p><?php _e('When user answers a "single choice" question:', 'watupro')?> <select name="single_choice_action" onchange="this.value == 'next' ? jQuery('#hideNextButton').show() : jQuery('#hideNextButton').hide()">
			...
			</select> <?php if($exam->single_page == WATUPRO_PAGINATE_ONE_PER_PAGE):?>
				<span id="hideNextButton" style='display:<?php echo (!empty($advanced_settings['single_choice_action']) and $advanced_settings['single_choice_action'] == 'next') ? 'inline':'none';?>'>
					<input type="checkbox" name="single_choice_action_hide_next" value="1" <?php if(!empty($advanced_settings['single_choice_action_hide_next'] )) echo 'checked';?>> <?php _e('Hide the next button', 'watupro');?> 
				</span>
			<?php endif;?><br>
			...
			<p id="storeOnlyLoggedIn" style='display:<?php echo empty($advanced_settings['dont_store_taking']) ? 'block' : 'none';?>'><input type="checkbox" name="store_taking_only_logged" <?php if(!empty($advanced_settings['store_taking_only_logged'])) echo 'checked'?> value="1" onclick="this.checked ? jQuery('#storeLoggedFirstLast').show() : jQuery('#storeLoggedFirstLast').hide();"> <?php printf(__("Store only the data / results of logged in users.", 'watupro'), WATUPRO_QUIZ_WORD);?>
				<span id="storeLoggedFirstLast" style='display:<?php echo (empty($advanced_settings['dont_store_taking']) and !empty($advanced_settings['store_taking_only_logged']))? 'inline' : 'none';?>'>
					<input type="checkbox" name="store_taking_first_last" <?php if(!empty($advanced_settings['store_taking_first_last'])) echo 'checked'?> value="1"> <?php printf(__("Store only the %s attempt of every user.", 'watupro'), '<select name="store_taking_first_last_select">
						<option value="first" '.sprintf((!empty($advanced_settings['store_taking_first_last']) and $advanced_settings['store_taking_first_last'] == 'first') ? 'selected' : '').'>'.__('First', 'watupro').'</option>
						<option value="last" '.sprintf((!empty($advanced_settings['store_taking_first_last']) and $advanced_settings['store_taking_first_last'] == 'last') ? 'selected' : '').'>'.__('Last', 'watupro').'</option>
					</select>');?>
				</span>			
			</p> 
			...
			<p><input type="checkbox" name="allow_public_details" value="1" <?php if(!empty($advanced_settings['allow_public_details'])) echo 'checked';?>> <?php printf(__('Allow everyone to see everyone\'s details on this %1$s. This is used when the %2$s shortcode is used with attributes public=1 and show_details=1. <a href="%3$s" target="_blank">Learn more</a>', 'watupro'),
			WATUPRO_QUIZ_WORD, '[watupro-takings]', 'https://blog.calendarscripts.info/filters-for-the-watupro-takings-shortcode/');?></p>
