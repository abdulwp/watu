...
function watupro_user_fields($user) {
    ...
    if(watupro_intel() and !current_user_can('manage_options')) {
    		if( !WatuPROIMultiUser :: check_access('usergroups_access', true)) return false;
    }
			...
			?>
			 <tr>
		      ...
		      <td>
		      	...
		      	$level = trim($level);
		      	if(is_array($user_diff_levels ?? null) and in_array($level, $user_diff_levels)) $selected="selected";
		      	else $selected="";?>
...
function watupro_group_assign() {
	...
	if(!empty($_GET['role'])) {
		...
		$filters_url .= "&role=".esc_attr($_GET['role']);
	}
	...
	// assign users
	if(!empty($_POST['assign']) and check_admin_referer('watupro_assign') ) {
		$_POST['uids'] = empty($_POST['uids']) ? array() : watupro_int_array($_POST['uids']);		
	...
	// mass-assign from CSV
	if( !empty($_POST['csv_assign']) and check_admin_referer('watupro_csv_assign')) {
        if(empty($_FILES['csv']['name'])) wp_die(__('Please upload file', 'watupro'));
        
        $num_assigned = 0;
        $num_column = absint($_POST['num_column']);
        $by = sanitize_text_field($_POST['by']);
        if( $by !== 'email' and $by !== 'login') $by = 'login';
        $delimiter = ',';
        ...
        if (($handle = fopen($_FILES['csv']['tmp_name'], "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 10000, $delimiter)) !== FALSE) {	    	  
                ...
                $login = $data[$num_column-1] ?? '';
                
                if( $login ) {
                    $user = get_user_by( $by, $login );
                    
                    if( !empty($user->ID) ) {
                        $user_groups = get_user_meta($user->ID, "watupro_groups", true);
                        if(!is_array($user_groups)) $user_groups = [ $user_groups ];      
