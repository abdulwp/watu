...
<<<<<<< HEAD
	// displays a selector to let user choose difficulty level on a quiz
	/* For now give up on this but maye be renewed later
	static function choose_difficulty_level($exam) {
		...
		if(empty($difficulty_levels) or !count($difficulty_levels)) return '';
		
		$output = '<p><label>'.__('Choose difficulty level', 'watupro').'</label> ';
		$output .= '<select id="watuproDiffLevel'.$exam->ID.'" name="watupro_diff_level">';
		$output .= '<option value="">'.__('All levels', 'watupro').'</option>';
		foreach($difficulty_levels as $dlev) {
			$dlev = trim($dlev);
			$selected = (!empty($_POST['waturo_diff_level']) and $_POST['watupro_diff_level'] == $dlev) ? ' selected' : '';
			$output .= '<option value="'.$dlev.'"'.$selected.'>'.$dlev.'</option>';
		}
		$output .= '</select>';
		$output .= '</p>';
		
		return $output;
	} // end choose difficulty level */
...
<<<<<<< HEAD
	...
	$all_answers = $wpdb->get_results($wpdb->prepare("SELECT * FROM ".WATUPRO_ANSWERS." WHERE question_id=%d AND question_id>0 ORDER BY sort_order", @$_GET['question']));
	
	$ans_type = ($action =='new' and empty($_GET['question'])) ? get_option('watupro_answer_type'): $question->answer_type;
	$answer_count = 4;
	if(($action == 'edit' or !empty($_GET['quesiton'])) and $answer_count < count($all_answers)) $answer_count = count($all_answers) ;	
=======
...
<<<<<<< HEAD
				watupro_redirect("admin.php?page=watupro_question&action=new&quiz=".$_GET['quiz']."&question=" . $question_id);
=======
...
<<<<<<< HEAD
	// mass delete questions
	if(!empty($_POST['mass_activate']) or !empty($_POST['mass_deactivate'])) {
		$qids = is_array($_POST['qids']) ? $_POST['qids'] : array(0);
=======
...
<<<<<<< HEAD
	...
	if(!empty($_GET['move'])) {
=======
	// mass add or remove tag
	if(!empty($_POST['mass_change_tags']) and check_admin_referer('watupro_questions') and empty($reusing_questions)) {
		$qids = is_array($_POST['qids']) ?  watupro_int_array($_POST['qids']) : array(0);
		$qid_sql = implode(", ", $qids);
		
		$tag = trim(sanitize_text_field($_POST['tag']));
		$tag = preg_replace('/[^a-zA-Z0-9_-]/','', $tag);
		
		if($_POST['mass_change_tags_mode'] == 'add') {
            $wpdb->query("UPDATE ".WATUPRO_QUESTIONS." SET tags =
            CASE
                WHEN tags LIKE '%|".$tag."|%' THEN tags
                WHEN tags = '' OR tags = '||' THEN '|".$tag."|'
                ELSE CONCAT( tags, '".$tag."|')
            END
			WHERE ID IN ($qid_sql) AND exam_id=".intval($_GET['quiz']));
		}
		
		if($_POST['mass_change_tags_mode'] == 'remove') {
            // Too many LIKE-s in the query. Better to run quesiton by question
            foreach($qids as $qid) {
                $tags = $wpdb->get_var($wpdb->prepare("SELECT tags FROM ".WATUPRO_QUESTIONS." WHERE ID=%d", $qid));                
                $tags = explode("|", $tags);
                ...
                $wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_QUESTIONS." SET tags=%s WHERE ID=%d", '|'.implode('|', $tags).'|', $qid));
            }
		}
	}
...
<<<<<<< HEAD
		$filter_sql .= $wpdb->prepare(" AND Q.difficulty_level=%s ", $_GET['filter_dlevel']);
=======
...
<<<<<<< HEAD
	$all_question = $wpdb->get_results("SELECT SQL_CALC_FOUND_ROWS Q.ID, Q.question, C.name as cat, 
		...
		WHERE Q.exam_id=".intval($_GET['quiz'])." $filter_sql ORDER BY Q.sort_order, Q.ID $limit_sql");
	
	if(empty($_GET['fix_sort_order'])) $num_questions = $wpdb->get_var("SELECT FOUND_ROWS()"); 
	else $num_questions = sizeof($all_question);
	
	if(empty($filter_sql) and !empty($_GET['fix_sort_order'])) {
...
=======
	...
	if(empty($_GET['fix_sort_order'])) $num_questions = $wpdb->get_var("SELECT FOUND_ROWS()"); 
	else $num_questions = count($all_question);
...
<<<<<<< HEAD
			if(!empty($_POST['ok'])) {
				if(!WTPCategory::add($_POST['name'], $_POST['description'], @$_GET['parent_id'], @$_POST['exclude_from_reports'])) {
					$error = __('Another category with this name already exists.', 'watupro');
					wp_die($error);
				}
				if(!$error) watupro_redirect("admin.php?page=watupro_question_cats&parent_id=" . @$_GET['parent_id']);
=======
...
<<<<<<< HEAD
				if(!WTPCategory::save($_POST['name'], $_GET['id'], $_POST['description'], @$_POST['exclude_from_reports'])) {
					$error = __('Another category with this name already exists.', 'watupro');
					wp_die($error);
				}
				if(!$error) watupro_redirect("admin.php?page=watupro_question_cats&parent_id=" . @$_GET['parent_id']);
...
<<<<<<< HEAD
			$cats = $wpdb->get_results("SELECT * FROM ".WATUPRO_QCATS." WHERE 1 $own_sql $parent_sql ORDER BY ID");	
=======
...
=======
// parses any occurencies of {{{answerto-...}}}, {{{feedbackto-}}}, or {{{pointsof-}}} mask.
// this mask is used to include the answer of a specific question
function watupro_parse_answerto($content, $taking_id, $exam) {
	global $wpdb;
	
	if(!strstr($content, '{{{answerto-') and !strstr($content, '{{{feedbackto-') and !strstr($content, '{{{pointsof-')) return $content;
	
	$advanced_settings = unserialize(stripslashes($exam->advanced_settings));
	
	// select all user answers of this taking
	$answers = $wpdb->get_results($wpdb->prepare("SELECT tA.answer as answer, tA.question_id as question_id, 
		tQ.is_survey as is_survey, tA.is_correct as is_correct, tA.points as points
...
=======
	...
	// now parse points of
	$matches = array();
	preg_match_all("/{{{pointsof-([^}}}])*}}}/", $content, $matches);
	
	foreach($matches[0] as $cnt=>$match) {
        
		// extract the question number
		$qid = str_replace('{{{pointsof-','',$match);
		$qid = str_replace('}}}','',$qid);
		
		$answer = array_filter($answers, function($a) use ($qid)  {
            if($a->question_id === $qid) return true;
            return false;
		});
		$answer = reset($answer);
		
		// gone through all answers but could not find any? replace with empty	
		$content = str_replace('{{{pointsof-'.$qid.'}}}', $answer->points + 0, $content);
	} // end foreach matches
