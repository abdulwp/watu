...
=======
	...
	// separate to-do quizzes in another array to make it easier to handle the new behavior of My Quizzes page
	$todo_quizzes = [];
	foreach($my_exams as $exam) {
		if(!$exam->is_taken) $todo_quizzes[] = $exam;
	}
	
	// regroup $todo_quizzes by category?
	$myquizzes_grouped_by_cat = get_option('watupro_myquizzes_grouped_by_cat');
	if($myquizzes_grouped_by_cat == 1) {
		// fill categories array and sort it accordingly to the setting
		$cats = [];
		
		foreach($todo_quizzes as $todo_quiz) {
			if(!isset($cats[$todo_quiz->cat_id])) $cats[$todo_quiz->cat_id] = ['id' => $todo_quiz->cat_id, 'name' => (empty($todo_quiz->cat) ? __('Uncategorized') : $todo_quiz->cat)];			
		}
      ...
      uasort($cats, function($a, $b) {
	   ...
	   // now group quizzes in cats
	   foreach($cats as $id => $cat) {
	   	$cat_quizzes = [];
	   	foreach($todo_quizzes as $todo) {
	   		if($todo->cat_id == $id) $cat_quizzes[] = $todo;
	   	}
	   	
	   	$cats[$id]['quizzes'] = $cat_quizzes;
	   }
	} // end grouping quizzes in categories
...
=======
		...
		$advanced_settings['single_choice_action_hide_next'] = empty($_POST['single_choice_action_hide_next']) ? 0 : 1;
		...
		$advanced_settings['play_levels'] = empty($_POST['play_levels']) ? '' : watupro_int_array($_POST['play_levels']); // restrict to levels from the play plugin
		$advanced_settings['play_badges'] = empty($_POST['play_badges']) ? '' : watupro_int_array($_POST['play_badges']); // restrict to levels from the play plugin
>>>>>>> branch/6.7.2
...
function watupro_override_atts(&$exam, $attr) {
	...
	// set the show_last result attribute so we can use it in submit_exam to clear the watupro_retake_mode_X user meta
	if(!empty($attr['show_last_result'])) $exam->attr_show_last_result = 1;
