...
function watupro_shortcode( $attr ) {
	...
	// allows you to run checks & DB updates on each loading of the quiz shortcode
	do_action('watupro_shortcode_before_select_quiz', $exam_id);
	...
	if(class_exists('WTPReports') and (strstr($exam->final_screen, '[watupror-pie-chart') or strstr($exam->final_screen, '[watupror-performance-chart'))) {		
		...
		if($exam->no_ajax) WTPReports::print_scripts();
		else WTPReports::enqueue_scripts();
	}
	...
	// display latest result instead of the quiz itself? 
	if(!empty($attr['show_last_result']) and !empty($user_ID)) {
		// try again pressed? 
		if(!empty($_POST['watupro_retake'])) {
			update_user_meta($user_ID, 'watupro_retake_mode_'.$exam->ID, "1");
			watupro_redirect(home_url( $wp->request ));
		}		
		
		if(empty($_POST['watupro_retake']) and get_user_meta($user_ID, 'watupro_retake_mode_'.$exam->ID, true) != "1") {
			// is there a last result?
			$result_exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM ".WATUPRO_TAKEN_EXAMS." 
				WHERE user_id=%d AND exam_id=%d AND in_progress=0", $user_ID, $exam->ID));
				
			if($result_exists) {
				$content = do_shortcode('[watupro-result what="details" quiz_id='.$exam->ID.' user_id='.$user_ID.']');
				
				$content .= '<div id="watupro-retake-button" class="watupro_buttons flex">
					<form method="post">
						<p><input type="submit" value="'.__('Try again', 'watupro').'"></p>
						<input type="hidden" name="watupro_retake" value=1>
					</form>
				</div>';
				return $content;
			}
		}		
	} // end if $attr['show_last_result']
...
// outputs my quiz settings in any post or page
function watupro_mysettings_code($attr) {
	$content = '';
	if(!is_user_logged_in()) return __('This content is only for logged in users', 'watupro');
	watupro_vc_scripts();
	
	ob_start();	
	watupro_my_options(true);
	$content = ob_get_contents();
	ob_end_clean();
	return $content;
}
...
function watupro_result($atts) {
	global $wpdb, $user_ID;
	$quiz_id = empty($atts['quiz_id']) ? 0 : intval($atts['quiz_id']);
	$user_id = empty($atts['user_id']) ? $user_ID : intval($atts['user_id']);
	...
	$result = $wpdb->get_row($wpdb->prepare("SELECT tT.ID as ID, tT.points as points, tT.percent_correct as percent_correct, 
        tT.percent_points as percent_points,
	...
	switch($what) {
		...
		case 'details': $content = $result->details; break;
		case 'points':
		default:
...
function watupro_retake_button($atts) {
	...
	$post_id = !empty($_POST['post_id']) ? intval($_POST['post_id']) : $post->ID;	
...
function watupro_multiquiz($atts, $content = '') {
	...
	// if only latest attempts are used we have to limit by taking IDs
	$tid_sql = "";
	if(!empty($atts['use_only_latest_attempts'])) {
	   $taking_ids = $wpdb->get_results($wpdb->prepare("SELECT MAX(ID) as max_id 
	     FROM ".WATUPRO_TAKEN_EXAMS. " WHERE in_progress=0 AND user_id=%d
	     AND exam_id IN (" .implode(',', $quiz_ids). ")
	     GROUP BY exam_id", $user_id));
	    
	   $tids = array_column($taking_ids, 'max_id');  
	   if(!empty($tids)) $tid_sql = " AND ID IN (".implode(',', $tids).") ";  
	}
	...
	if(!empty($atts['condition_points'])) {
		$points = $wpdb->get_var($wpdb->prepare("SELECT SUM(points) FROM ".WATUPRO_TAKEN_EXAMS." 
			WHERE user_id=%d AND in_progress=0 $tid_sql 
			AND exam_id IN (" .implode(',', $quiz_ids). ")", $user_id));
	...
	if(!empty($atts['condition_percent_correct'])) {
		$percent = $wpdb->get_var($wpdb->prepare("SELECT AVG(percent_correct) FROM ".WATUPRO_TAKEN_EXAMS." 
			WHERE user_id=%d AND in_progress=0 $tid_sql AND exam_id IN (" .implode(',', $quiz_ids). ")", $user_id));
...
function watupro_answer($atts) {
	...
	// let's allow user ID to be passed or taken from certificate
	if(!empty($atts['user_id']) and $atts['user_id'] == 'certificate') $user_id = intval($_POST['watupro_certificate_user_id']);		
...
// points collected on a single question
// $atts[question_id] - the question ID
// $atts[user_id] - optional user ID otherwise logged in user
// $atts[taking_id] - optional taking ID otherwise latest taking 
function watupro_question_points($atts) {
    global $wpdb;
	$question_id = empty($atts['question_id']) ? 0 : intval($atts['question_id']);
	$user_id = empty($atts['user_id']) ? 0 : intval($atts['user_id']);
	...
	if(empty($taking_id)) {
        $taking_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM ".WATUPRO_TAKEN_EXAMS." 
            WHERE user_id=%d ORDER BY ID DESC LIMIT 1", $user_id));
	}
	
	$points = $wpdb->get_var($wpdb->prepare("SELECT points FROM ".WATUPRO_STUDENT_ANSWERS." 
        WHERE question_id=%d AND taking_id=%d", $question_id, $taking_id));
        
    return $points;
}

// memory shortcode. The wrapped content will be shown for X seconds before showing the question.
// $atts['seconds'] - for how many seconds to be shown 
function watupro_memory($atts, $content) {	
	$id = empty($atts['id']) ? 0 : intval($atts['id']);
	$seconds = empty($atts['seconds']) ? 5 : intval($atts['seconds']);
	
	$html = '<span id="watuproMemory'.$id.'" class="watupro-memory watupro-memory-seconds-'.$seconds.'">'.do_shortcode($content).'</span>';
