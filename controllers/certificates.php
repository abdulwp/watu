...
=======
	...
	if(!empty($_POST['ok'])) {
	   ...
	   $avg_percent = intval($_POST['avg_percent']);
		...
		$bgcolor = empty($_POST['bgcolor']) ? '#FFFFFF' : sanitize_text_field($_POST['bgcolor']);
	}
		...
		case 'add':
			if(!empty($_POST['ok']) and check_admin_referer('watupro_certificate')) {
				$wpdb->query($wpdb->prepare("INSERT INTO ".WATUPRO_CERTIFICATES." (title, html, require_approval, 
					...
					expiration_mode, expiration_date, var_text, avg_on_each_quiz, fee, bgcolor)
					VALUES (%s, %s, %d, %d, %d, %s, %s, %d, %d, %s, %d, %d, %d, %s, %s, %s, %s, %s, %d, %f, %s)", 
					...
					$has_expiration, $expiration_period, $expired_message, $expiration_mode, $expiration_date, $var_text, $avg_on_each_quiz, $fee, $bgcolor));
>>>>>>> branch/6.7.2
				$cid = $wpdb->insert_id;
...
=======
			if(!empty($_POST['ok']) and check_admin_referer('watupro_certificate')) {
				$wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_CERTIFICATES." SET
					...
					avg_on_each_quiz = %d, fee = %f, bgcolor=%s
					...
					$bgcolor, intval($_GET['id'])));
				do_action('watupro-certificate-saved', intval($_GET['id']));	
>>>>>>> branch/6.7.2
				echo "<meta http-equiv='refresh' content='0;url=admin.php?page=watupro_certificates' />"; 
				exit;
			}
...
=======
				...
				$obfuscate_certificate_ids = empty($_POST['obfuscate_certificate_ids']) ? 0 : 1;
				...
				update_option('watupro_obfuscate_certificate_ids', $obfuscate_certificate_ids);
>>>>>>> branch/6.7.2
			}		
...
=======
			...
			$ob = $_GET['ob'] ?? 'title';
			if($ob !== 'title' AND $ob !== 'ID') $ob = 'title';
			$dir = sanitize_text_field($_GET['dir'] ?? 'ASC');			
			$odir = $dir === 'ASC' ? 'DESC' : 'ASC';
			
			$certificates=$wpdb->get_results("SELECT * FROM ".WATUPRO_CERTIFICATES." $own_sql ORDER BY $ob $dir");
>>>>>>> branch/6.7.2
...
=======
		// if taking ID is obfuscated in new versions		
		$param_taking_id = (is_numeric($_GET['taking_id']) and get_option('watupro_obfuscate_certificate_ids') != '1') ? intval($_GET['taking_id']) : convert_uudecode (base64_decode ($_GET['taking_id']));
		
		// find taking 
		$taking = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".WATUPRO_TAKEN_EXAMS." WHERE ID=%d", $param_taking_id));
...
=======
		...
		if(!is_user_logged_in() and empty($user_certificate->public_access) and !empty($user_certificate->user_id) and empty($public_certificates)) {
			watupro_redirect( wp_login_url(site_url("?watupro_view_certificate=1&taking_id=".sanitize_text_field($_GET['taking_id'])."&id=".intval($_GET['id']))) );
		}
...
=======
	   ...
	   $output = str_replace("%%DATE%%", $taken_date, $output);
	   $param_taking_id = pow($taking->ID, 3).substr(str_shuffle('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'), 1, 3 );
	   $certificate_url = site_url("?watupro_view_certificate=1&taking_id=".$param_taking_id."&id=".$certificate->ID);
	   $output = str_replace("%%URL%%", $certificate_url, $output);
...
=======
	 ...
	 $bgcolor = empty($certificate->bgcolor) ? 'white' : $certificate->bgcolor;
>>>>>>> branch/6.7.2
...
/**
* Attach certificates to email messages
**/
function watupro_maybe_attach_certificate( $certificate_id, $taking_id ) {
    global $wpdb;
    ...
    if(!empty($certificate_id) and $generate_pdf_certificates == "1" and $attach_certificates) {
        ...
        foreach($certificate_id as $cert_id) {
            ...
            $_GET['id'] = $cert_id;	
            $_GET['taking_id'] = $taking_id;
            if( get_option('watupro_obfuscate_certificate_ids') == '1' ) $_GET['taking_id'] = WatuPROCertificate :: obfuscate($taking_id);
