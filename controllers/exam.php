...
function watupro_exam() {
	...
	if(isset($_REQUEST['submit'])) {
		...
		$advanced_settings['contact_fields']['start_button'] = rawurlencode(sanitize_text_field($_POST['ask_for_start_button']));
		...
		$advanced_settings['progress_bar_mode'] = empty($_POST['progress_bar_mode']) ? 0 : 1;
		$advanced_settings['show_category_paginator'] = empty($_POST['show_category_paginator']) ? 0 : 1;
		...
		$advanced_settings['store_taking_first_last'] = '';
		if(!empty($advanced_settings['store_taking_only_logged']) and !empty($_POST['store_taking_first_last'])) {
			$advanced_settings['store_taking_first_last']  = sanitize_text_field($_POST['store_taking_first_last_select']);
		}
		...
		$advanced_settings['grades_by_percent_mode'] = sanitize_text_field(@$_POST['grades_by_percent_mode']);
		...
		// allow public to see someone's quiz details
		$advanced_settings['allow_public_details'] = empty($_POST['allow_public_details']) ? 0 : 1;
		...
		// default final screen and email output
		$advanced_settings['use_default_final_screen'] = empty($_POST['use_default_final_screen']) ? 0 : 1;
		$advanced_settings['use_default_email_output'] = empty($_POST['use_default_email_output']) ? 0 : 1;
		...
		if(!empty($_POST['completion_criteria'])) {
		   ...
		   $_POST['completion_grades'] = $_POST['completion_grades'] ?? [];
	...
	$advanced_settings = array('play_levels' => '', 'play_badges' => '');
...
function watupro_exams() {
	...
	// other mass changes
	if(!empty($_POST['other_mass_changes']) and check_admin_referer('watupro_exams')) {
		$qids = is_array($_POST['qids']) ?  watupro_int_array($_POST['qids']) : array(0);
		$qid_sql = implode(", ", $qids);
		
		$is_rtl = intval($_POST['is_rtl']);
		
		// RTL change should be done one by one because it's in advanced settings
		foreach($qids as $qid) {
			$ex = $wpdb->get_row($wpdb->prepare("SELECT ID, advanced_settings FROM ".WATUPRO_EXAMS." WHERE ID=%d", $qid));
			$exadv = unserialize(stripslashes($ex->advanced_settings));
			$exadv['is_rtl'] = $is_rtl;
			
			$wpdb->query($wpdb->prepare("UPDATE ".WATUPRO_EXAMS." SET advanced_settings = %s WHERE ID=%d", serialize($exadv), $qid));
		}
	}
	...
	if(get_option('watupro_auto_db_cleanup') == '1') {		
		...
		$external_sql = apply_filters('watupro-external-cleanup-conditions', '', $config);
		
		if($days > 0) {
			$wpdb->query($wpdb->prepare("DELETE FROM ".WATUPRO_TAKEN_EXAMS." 
				WHERE date < '".date("Y-m-d", current_time('timestamp'))."' - INTERVAL %d DAY $points_sql $percent_sql $external_sql", $days));
	...
	if(get_option('watupro_auto_db_blankout') == '1') {	
		...
		$external_sql = apply_filters('watupro-external-blankout-conditions', '', $config);
	...
	if(isset($_GET['cat_id']) and $_GET['cat_id']!= -1) {
		...
		$filter_params .= "&cat_id=".intval($_GET['cat_id']);
	}
	if(isset($_GET['is_active']) and $_GET['is_active']!= -1) {
		$filter_sql .= $wpdb->prepare(" AND Q.is_active = %d ", intval($_GET['is_active']));
		$filter_params .= "&is_active=".intval($_GET['is_active']);
	}
	if(!empty($_GET['title'])) {
