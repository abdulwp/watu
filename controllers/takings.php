...
function watupro_takings($in_shortcode = false, $atts = null) {	
	...
	if($in_shortcode) {
		...
		if(!empty($atts['points']) and empty($_GET['points'])) $_GET['points'] = floatval($atts['points']);
		if(!empty($atts['points_filter']) and empty($_GET['pointsf'])) $_GET['pointsf'] = $atts['points_filter'];
		if(!empty($atts['percent_points']) and empty($_GET['percent_points'])) $_GET['percent_points'] = intval($atts['percent_points']);
		if(!empty($atts['percent_points_filter']) and empty($_GET['percent_pointsf'])) $_GET['percent_pointsf'] = $atts['percent_points_filter'];
	...
	if(!empty($_GET['points'])) {
		...
		switch($_GET['pointsf']) {
			case 'less': $filters[]=$wpdb->prepare(" points<%f ", floatval($_GET['points'])); break;
			case 'more': $filters[]=$wpdb->prepare(" points>%f ", floatval($_GET['points'])); break;
			case 'equals':
			default: $filters[]=$wpdb->prepare(" points=%f ", floatval($_GET['points'])); break;
		}
	}
	...
	if(!empty($_GET['percent_correct'])) {
		...
		switch($_GET['percentf']) {
			case 'less': $filters[]=$wpdb->prepare(" percent_correct < %d ", intval($_GET['percent_correct'])); break;
			case 'more': $filters[]=$wpdb->prepare(" percent_correct > %d ", intval($_GET['percent_correct'])); break;
			case 'equals':
			default: $filters[]=$wpdb->prepare(" percent_correct = %d ", intval($_GET['percent_correct'])); break;
		}
	}
	
	// % of points	
	if(!empty($_GET['percent_points'])) {
		$_GET['percent_points'] = intval($_GET['percent_points']);
		switch($_GET['percent_pointsf']) {
			case 'less': $filters[]=$wpdb->prepare(" percent_points < %d ", intval($_GET['percent_points'])); break;
			case 'more': $filters[]=$wpdb->prepare(" percent_points > %d ", intval($_GET['percent_points'])); break;
			case 'equals':
			default: $filters[] = $wpdb->prepare(" percent_points = %d ", intval($_GET['percent_points'])); break;
		}
	}
	...
	if(!empty($_GET['namaste_course_id']) and !empty($namaste_courses)) {
		...
		// tags?
		$ntag_sql = '';
		if( !empty( $_GET['namaste_tags'] ) ) {
			$ntags = array_map( 'trim', explode (',', sanitize_text_field( $_GET['namaste_tags']  ) ) );			
			
			foreach( $ntags as $cnt => $ntag ) {			  	
				$ntag_sql .= $wpdb->prepare( " AND ( tags LIKE %s OR tags LIKE %s OR tags LIKE %s OR tags LIKE %s ) ", $ntag, $ntag.',%', '%,'.$ntag, '%,'.$ntag.',%' );  
			}			 
		}		
		
		$namaste_uids1 = $wpdb->get_results($wpdb->prepare("SELECT user_id FROM ".NAMASTE_STUDENT_COURSES." 
			WHERE course_id=%d AND (status='enrolled' OR status='completed') $ntag_sql ", intval($_GET['namaste_course_id'])));
	...
	if((!empty($_POST['cleanup']) or !empty($_POST['blankout'])) and check_admin_referer("watupro_cleanup") and empty($atts['public']) and empty($class_manager_sql)) {
		...
		if(!empty($_POST['cleanup'])) {		
			...
			$wpdb->query($wpdb->prepare("DELETE FROM ".WATUPRO_USER_FILES." WHERE taking_id IN (SELECT ID FROM ".WATUPRO_TAKEN_EXAMS." WHERE exam_id=%d AND in_progress=1) $takings_id_sql ", $exam->ID));
		}
	...
	$get_percent_points = empty($_GET['percent_points']) ? '' : esc_attr($_GET['percent_points']);
	$get_percent_pointsf = empty($_GET['percent_pointsf']) ? '' : esc_attr($_GET['percent_pointsf']);
	...
	$filters_url="dn=".$get_dn."&dnf=".$get_dnf."&email=".$get_email."&emailf=".
		...
		"&percent_points=".$get_percent_points.'&percent_pointsf='.$get_percent_pointsf;
...
} // end watupro_questions_feedback

/**
* Import takings
**/
function watupro_import_takings() : void {
    global $wpdb;
    
    $quiz = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".WATUPRO_EXAMS." WHERE ID=%d", intval($_GET['exam_id'])));		
    if(empty($quiz)) wp_die(__('Invalid link', 'watupro'));
    
    $result = false;
    if(!empty($_POST['watupro_import']) and check_admin_referer('watupro_import_takings')) {
        $num_rows = WTPRecord :: import($quiz);
        
        $message = sprintf(__('Imported %d records.', 'watupro'), $num_rows);
        $result = true;
    } // end if importing
    
    if(@file_exists(get_stylesheet_directory().'/watupro/import-takings.html.php')) require get_stylesheet_directory().'/watupro/import-takings.html.php';
	else require WATUPRO_PATH."/views/import-takings.html.php";
} // end watupro_import_takings
